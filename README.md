...
docker run -it daraff/load-test -d 60 -a 25 -f "https://example.com/urls.txt"
```
...
docker run -it -v "$PWD":/usr/src/app -w /usr/src/app daraff/load-test -d 60 -a 25 -f "https://example.com/urls.txt" -o result.json
```